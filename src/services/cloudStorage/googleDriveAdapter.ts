 ...
 * Implements cloud storage using Google Drive API
 */
...
export class GoogleDriveAdapter implements CloudStorageAdapter {
  ...
  async uploadFile(
    ...
    try {
      ...
      if (file instanceof ArrayBuffer) {
        ...
        // This handles Blob or other file-like objects
        if (file instanceof Blob) {
          const arrayBuffer = await file.arrayBuffer();
          base64Data = this.arrayBufferToBase64(arrayBuffer);
        } else {
          // Last resort: assume it's ArrayBuffer-like
      ...
      return {
        fileId: result.id,
        fileUrl: result.webViewLink || result.webContentLink,
        fileName,
        folderPath,
      };
    } catch (error) {
  ...
  async getFileUrl(fileId: string): Promise<string> {
    try {
      ...
      logger.error('Error getting Google Drive file URL:', error);
  ...
  private async findFolder(
    ...
    try {
      ...
      logger.error('Error finding Google Drive folder:', error);
      return null;
    }
  }
