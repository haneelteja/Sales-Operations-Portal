-- Merge duplicate Intercity Bachupally customer records.
-- The "keeper" is the customer_id that has payment transactions (correct record).
-- All sales, orders, invoices, factory payables, followup notes, and transport
-- expenses from the duplicate are re-assigned to the keeper, then the duplicate
-- customer row is deleted.
--
-- Safe to re-run: raises an exception and rolls back if preconditions are not met.
...
BEGIN
  ...
  -- ── 1. Find the keeper: has payment transactions ─────────────────────────────
  ...
  IF v_keeper_id IS NULL THEN
    RAISE EXCEPTION 'No Intercity Bachupally customer found with payment records. Aborting.';
  END IF;

  -- ── 2. Find the duplicate: same name+branch, different id, no payments ───────
  ...
  IF v_dup_id IS NULL THEN
    RAISE EXCEPTION 'Could not find a second Intercity Bachupally customer record. Aborting.';
  END IF;
