-- Fix get_receivables_summary to group by (client_name, branch) instead of customer_id.
-- Previously grouped by c.id which caused:
--   1. Duplicate rows when a client has multiple customer rows (different SKUs/pricing dates).
--   2. Split-UUID payments: payment on UUID-A and sales on UUID-B showed as separate rows,
--      inflating outstanding and misreporting payment count.
-- Now aggregates all transactions across all UUIDs for a (client_name, branch) pair.
-- The representative customer_id returned is the UUID with the most transactions.
...
AS $$
  SELECT
    ...
    -- This ensures the ledger drawer and followup drawer default to the most active record.
    (
