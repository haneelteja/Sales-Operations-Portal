-- Include transactions from ALL customer_ids for a client+branch pair in both RPCs,
-- not just the active customer_id. This fixes the case where payments were recorded
-- under a deactivated duplicate customer record and were being silently excluded,
-- causing the outstanding balance to appear higher than reality.
-- A client+branch is still only returned when at least one customer record is active.
...
GRANT EXECUTE ON FUNCTION public.get_receivables_summary() TO anon;
